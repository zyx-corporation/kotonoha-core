...
| `SLS-5.4.5` Evidence binding (Phase E) | `EvidenceRef`, `EvidenceRole`, `EvidenceBinding`, `EvidenceBindingReport`, `bind_evidence_minimal` | Minimal evidence binding structs preserving classification without reclassifying. [`src/rde_evidence.rs`](../src/rde_evidence.rs). Design gate: [`docs/evidence-binder-design.md`](evidence-binder-design.md). |
| `SLS-5.7` Meta-RDE audit (Phase F) | `MetaRdeSeverity`, `PipelinePhase`, `MetaRdeFinding`, `MetaRdeReport`, `run_meta_rde_minimal` | Minimal recursive audit layer. `Blocking` severity = review obligation, not auto-rejection. [`src/meta_rde.rs`](../src/meta_rde.rs). |
| *(review handoff)* Combined report (Phase G) | `RdeCombinedReviewReport`, `build_combined_review_report`, `render_combined_review_markdown` | Markdown review artifact integrating evidence and Meta-RDE findings. Not a decision object. [`src/rde_report.rs`](../src/rde_report.rs). |
| `SLS-5.9` Human Review Decision (Phase H) | `HumanReviewDecision`, `HumanReviewDecisionKind`, `HumanReviewReason`, `HumanReviewAction`, `create_human_review_decision` | Minimal decision structs. Approve/Reject require reasons; RequestRevision/Defer allowed without. External audit record, not automatic engine. [`src/human_review.rs`](../src/human_review.rs). Design gate: [`docs/human-review-decision-model.md`](human-review-decision-model.md). |
