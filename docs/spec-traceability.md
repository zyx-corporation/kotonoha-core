...
| `SLS-5.7` Meta-RDE audit (Phase F) | `MetaRdeSeverity`, `PipelinePhase`, `MetaRdeFinding`, `MetaRdeReport`, `run_meta_rde_minimal` | Minimal recursive audit layer. `Blocking` severity = review obligation, not auto-rejection. [`src/meta_rde.rs`](../src/meta_rde.rs). Design gate: [`docs/meta-rde-design.md`](meta-rde-design.md). |
