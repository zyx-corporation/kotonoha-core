...
| 項目 | 状況 |
| --- | --- |
| **`lineage`／`rde`／`interchange`** のライブラリテスト | 各 `*.rs` モジュール内に **計 23 本**（許容／拒否／strict・**エンベロープと RDE 形状の Negative** を中心） |
...
| 2026-05-11 | OSS 側でライブラリ単体を **23 本**に拡張（`interchange`／`rde` の Negative 中心、テスト先行で既存検証と整合）。 |